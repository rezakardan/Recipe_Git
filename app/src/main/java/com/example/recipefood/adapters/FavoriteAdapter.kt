...
class FavoriteAdapter @Inject constructor() :
    RecyclerView.Adapter<FavoriteAdapter.RecentViewHolder>() {
    ...
    private var recentFoods = emptyList<FavoriteEntity>()
    ...
    inner class RecentViewHolder(item: View) : RecyclerView.ViewHolder(item) {
        ...
        fun onBind(oneItem: FavoriteEntity) {
            ...
            oneItem.favoriteResponse.let { result->
            ...
            binding.recipeNameTxt.text = result.title



            val htmlFormater=HtmlCompat.fromHtml(result.summary!!,HtmlCompat.FROM_HTML_MODE_COMPACT)
            binding.recipeDesc.text = htmlFormater

            binding.recipeLikeTxt.text = result.aggregateLikes.toString()
            ...
            if (result.readyInMinutes!! >=60){
                ...
                val oclock=(result.readyInMinutes)/60

                val minutes=(result.readyInMinutes)%60
                binding.recipeTimeTxt.text = "${oclock}:${minutes}"

            }else{

                binding.recipeTimeTxt.text = result.readyInMinutes.toString()+"min"
            ...
            binding.recipeHealthTxt.text = result.healthScore.toString()
